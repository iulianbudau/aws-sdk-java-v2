...
public abstract class Condition extends UnitOfExpression {
    ...
    public AndCondition and(Condition that) {
        return new AndCondition(this, that.atomic() ? that : wrapCondition(that));
    }
    ...
    public OrCondition or(Condition that) {
        return new OrCondition(this, that.atomic() ? that : wrapCondition(that));
    }

    /**
     * A condition is considered "atomic" if appending an additional AND condition would not alter the evaluation order of the
     * original condition; false otherwise.  For example, "a == b AND c == d" is atomic, but "a == b OR c == d" is not.
     */
    abstract boolean atomic();

    /**
     * Returns the precedence of this condition. See
     * <p>
     * <a href="http://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Expressions.SpecifyingConditions.html">ConditionExpressionReference</a>
     */
