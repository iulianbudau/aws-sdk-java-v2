...
public class GetItemWithResponseTest extends LocalDynamoDbSyncTestBase {
    ...
    @Test
    public void extensionEnabledClient_getAndGetWithResponse_areEquivalent() {
        DynamoDbEnhancedClient extensionClient = DynamoDbEnhancedClient.builder()
                                                                       .dynamoDbClient(getDynamoDbClient())
                                                                       .extensions(AutoGeneratedTimestampRecordExtension.create())
                                                                       .build();
        DynamoDbTable<Record> extensionTable = extensionClient.table(getConcreteTableName(TEST_TABLE_NAME), TABLE_SCHEMA);
        Record original = new Record().setId(501).setStringAttr1("abc");
        extensionTable.putItem(original);

        Record getResult = extensionTable.getItem(r -> r.key(k -> k.partitionValue(501)));
        GetItemEnhancedResponse<Record> response = extensionTable.getItemWithResponse(r -> r.key(k -> k.partitionValue(501)));
        assertThat(getResult).isEqualTo(response.attributes());
    }
