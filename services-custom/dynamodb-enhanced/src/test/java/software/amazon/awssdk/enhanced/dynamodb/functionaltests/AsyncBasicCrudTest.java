...
public class AsyncBasicCrudTest extends LocalDynamoDbAsyncTestBase {
    ...
    @Test
    public void extensionEnabledClient_withUnsupportedCustomSchema_putAndUpdateDoNotThrow() {
        DynamoDbEnhancedAsyncClient extensionClient = DynamoDbEnhancedAsyncClient.builder()
                                                                                 .dynamoDbClient(getDynamoDbAsyncClient())
                                                                                 .extensions(AutoGeneratedTimestampRecordExtension.create())
                                                                                 .build();
        DynamoDbAsyncTable<Map<String, AttributeValue>> customTable =
            extensionClient.table(getConcreteTableName("crud-custom-table"), new UnsupportedConverterTableSchema());
        customTable.createTable(r -> r.provisionedThroughput(getDefaultProvisionedThroughput())).join();
        ...
        customTable.updateItem(item).join();

        Map<String, AttributeValue> persisted = customTable.getItem(r -> r.key(k -> k.partitionValue("custom-1"))).join();
        assertThat(persisted, is(notNullValue()));
        getDynamoDbAsyncClient().deleteTable(r -> r.tableName(getConcreteTableName("crud-custom-table"))).join();
    }
}
