...
public class AsyncGetItemWithResponseTest extends LocalDynamoDbAsyncTestBase {
    ...
    public void extensionEnabledClient_getAndGetWithResponse_areEquivalent() {
        DynamoDbEnhancedAsyncClient extensionClient = DynamoDbEnhancedAsyncClient.builder()
                                                                                 ...
                                                                                 .build();
        DynamoDbAsyncTable<Record> extensionTable = extensionClient.table(getConcreteTableName(TEST_TABLE_NAME), TABLE_SCHEMA);
        Record original = new Record().setId(777).setStringAttr1("abc");
        extensionTable.putItem(original).join();

        Record getResult = extensionTable.getItem(r -> r.key(k -> k.partitionValue(777))).join();
        GetItemEnhancedResponse<Record> response =
            extensionTable.getItemWithResponse(r -> r.key(k -> k.partitionValue(777))).join();
