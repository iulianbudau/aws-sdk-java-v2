...
public class AsyncBatchGetItemTest extends LocalDynamoDbAsyncTestBase {
    ...
    public static final class ImmutableRecord {
        ...
        private ImmutableRecord(Builder b) { this.id = b.id; this.value = b.value; }
        @DynamoDbPartitionKey
    ...
    private final DynamoDbAsyncTable<BeanRecord> mappedBeanTable =
        enhancedAsyncClient.table(getConcreteTableName("table-name-bean"), TableSchema.fromBean(BeanRecord.class));
    private final DynamoDbAsyncTable<ImmutableRecord> mappedImmutableTable =
        enhancedAsyncClient.table(getConcreteTableName("table-name-immutable"), TableSchema.fromImmutableClass(ImmutableRecord.class));
    ...
    public void getRecordsFromMultipleTables_withExtensionEnabled_doesNotThrowAndMatches() {
        ...
        DynamoDbAsyncTable<Record1> extensionMappedTable1 = extensionClient.table(tableName1, TABLE_SCHEMA_1);
        DynamoDbAsyncTable<Record2> extensionMappedTable2 = extensionClient.table(tableName2, TABLE_SCHEMA_2);
        ...
        BatchGetResultPagePublisher publisher = extensionClient.batchGetItem(request);
        List<BatchGetResultPage> results = drainPublisher(publisher, 1);
        assertThat(results.size(), is(1));

        BatchGetResultPage page = results.get(0);
        List<Record1> record1List = page.resultsForTable(extensionMappedTable1);
        assertThat(record1List.size(), is(2));
        assertThat(record1List, containsInAnyOrder(RECORDS_1.get(0), RECORDS_1.get(1)));

        List<Record2> record2List = page.resultsForTable(extensionMappedTable2);
        assertThat(record2List.size(), is(2));
        assertThat(record2List, containsInAnyOrder(RECORDS_2.get(0), RECORDS_2.get(1)));
    }
    ...
    public void getRecordsFromMixedSchemaTables_inSingleRequest_matchesExpected() {
        ...
        bean.setId(10);
        bean.setValue("bean");
        mappedBeanTable.putItem(bean).join();
        ImmutableRecord immutable = ImmutableRecord.builder().id(20).value("immutable").build();
        mappedImmutableTable.putItem(immutable).join();

        BatchGetResultPagePublisher publisher = enhancedAsyncClient.batchGetItem(r -> r.readBatches(
        ...
        List<BatchGetResultPage> pages = drainPublisher(publisher, 1);
        assertThat(pages.size(), is(1));
        assertThat(pages.get(0).resultsForTable(mappedBeanTable).get(0).getValue(), is("bean"));
        assertThat(pages.get(0).resultsForTable(mappedImmutableTable).get(0).value(), is("immutable"));
    }
