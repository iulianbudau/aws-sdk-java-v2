...
public class BasicCrudTest extends LocalDynamoDbSyncTestBase {
    ...
    public void extensionEnabledClient_withUnsupportedCustomSchema_putAndUpdateDoNotThrow() {
        ...
        customTable.putItem(item);
        customTable.updateItem(item);

        Map<String, AttributeValue> persisted = customTable.getItem(r -> r.key(k -> k.partitionValue("custom-1")));
        assertThat(persisted, is(notNullValue()));
        getDynamoDbClient().deleteTable(r -> r.tableName(getConcreteTableName("crud-custom-table")));
    }
}
