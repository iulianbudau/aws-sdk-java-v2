...
public class AsyncTransactGetItemsTest extends LocalDynamoDbAsyncTestBase {
    ...
    public void getRecordsFromMultipleTables_withExtensionEnabled_doesNotThrowAndMatches() {
        ...
        DynamoDbEnhancedAsyncClient extensionClient = DefaultDynamoDbEnhancedAsyncClient.builder()
                                                                                        ...
                                                                                        .build();
        DynamoDbAsyncTable<Record1> extensionMappedTable1 =
            extensionClient.table(getConcreteTableName("table-name-1"), TABLE_SCHEMA_1);
        DynamoDbAsyncTable<Record2> extensionMappedTable2 =
            extensionClient.table(getConcreteTableName("table-name-2"), TABLE_SCHEMA_2);

        TransactGetItemsEnhancedRequest request =
            TransactGetItemsEnhancedRequest.builder()
                                           .addGetItem(extensionMappedTable1, Key.builder().partitionValue(0).build())
                                           .addGetItem(extensionMappedTable2, Key.builder().partitionValue(0).build())
                                           .addGetItem(extensionMappedTable2, Key.builder().partitionValue(1).build())
                                           .addGetItem(extensionMappedTable1, Key.builder().partitionValue(1).build())
                                           .build();

        List<Document> results = extensionClient.transactGetItems(request).join();
        assertThat(results.size(), is(4));
        assertThat(results.get(0).getItem(extensionMappedTable1), is(RECORDS_1.get(0)));
        assertThat(results.get(1).getItem(extensionMappedTable2), is(RECORDS_2.get(0)));
        assertThat(results.get(2).getItem(extensionMappedTable2), is(RECORDS_2.get(1)));
        assertThat(results.get(3).getItem(extensionMappedTable1), is(RECORDS_1.get(1)));
    }
}
