...
public class AsyncTransactWriteItemsTest extends LocalDynamoDbAsyncTestBase {
    ...
    public void mixedCommands_returnValuesOnConditionCheckFailureSet_allConditionsFail() {
        ...
        try {
            enhancedAsyncClient.transactWriteItems(request).join();
            fail("Expected CompletionException to be thrown");
        } catch (CompletionException e) {
            assertThat(e.getCause(), instanceOf(TransactionCanceledException.class));
            List<CancellationReason> reasons = ((TransactionCanceledException) e.getCause()).cancellationReasons();
            assertThat(reasons.size(), is(4));
            reasons.forEach(r -> assertThat(r.item().isEmpty(), is(false)));
        }
    }
    ...
    public void transactWriteWithSupportedAndUnsupportedSchema_withAutoTimestampEnabled_succeeds() {
        ...
        customItem.put("payload",
                       AttributeValue.builder().m(singletonMap("inner", AttributeValue.builder().s("value").build())).build());

        extensionEnhancedAsyncClient.transactWriteItems(
            TransactWriteItemsEnhancedRequest.builder()
                                             .addPutItem(timestampedTable, new TimestampedRecord().setId("ts-1"))
                                             .addPutItem(unsupportedCustomSchemaTable, customItem)
                                             .build()).join();
    ...
    public void transactWriteWithSupportedAndUnsupportedSchema_withAutoTimestampEnabled_withResponse_succeeds() {
        ...
        TransactWriteItemsEnhancedResponse response = extensionEnhancedAsyncClient.transactWriteItemsWithResponse(
            TransactWriteItemsEnhancedRequest.builder()
                                             .returnConsumedCapacity(ReturnConsumedCapacity.TOTAL)
                                             .returnItemCollectionMetrics(ReturnItemCollectionMetrics.SIZE)
                                             .addPutItem(timestampedTable, new TimestampedRecord().setId("ts-1"))
